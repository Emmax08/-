...
// ====================================================================
// El bot asume que está sirviendo a la red 'maria'

// Endpoints para Códigos de Verificación
const CODES_API_ENDPOINT = 'https://report-bots-causas.duckdns.org/api/verification/codes/pending/maria'; 
const CODES_NOTIFICATION_BASE_URL = 'https://report-bots-causas.duckdns.org/api/verification/codes/mandado/maria'; 

// Endpoints para Mensajes del Bot (Actualizaciones de Reportes)
const MESSAGES_API_ENDPOINT = 'https://report-bots-causas.duckdns.org/api/verification/messages/pending/maria';
const MESSAGES_NOTIFICATION_BASE_URL = 'https://report-bots-causas.duckdns.org/api/verification/messages/mandado/maria';
...
export async function checkCodesEndpoint(conn, dbData) {
    ...
    try {
        ...
        const sendPromises = pendingCodes.map(async (codeEntry) => {
            ...
            if (!isUserInDB) {
                // Error: El usuario no ha interactuado con el Bot
            ...
            try {
                ...
                // Error: Baileys no pudo enviar el mensaje (ej. Bloqueo)
        ...
        if (sentCodeIds.length > 0) {
            ...
            try {
                ...
                // 🛑 CORRECCIÓN CLAVE: Fallar si la API no retorna 200/202 (ok)
                if (!notificationResponse.ok) {
                    throw new Error(`API central respondió con error HTTP: ${notificationResponse.status}`);
                }
                // Operación silenciosa si la respuesta es OK (200-299)
            } catch (notificationError) {
                // Operación silenciosa. Esto es la falla de red o del API. El mensaje SE REENVIARÁ.
            }
        }

    } catch (error) {
        // Operación silenciosa (Errores de fetch inicial)
    }
}
    ...
    try {
        ...
        if (sentMessageIds.length > 0) {
            ...
            try {
                ...
                // 🛑 CORRECCIÓN CLAVE: Si la API no retorna 200/202 (ok), forzamos el fallo
                if (!notificationResponse.ok) {
                    throw new Error(`API central respondió con error HTTP: ${notificationResponse.status}`);
                }
                // Operación silenciosa si la respuesta es OK (200-299)
